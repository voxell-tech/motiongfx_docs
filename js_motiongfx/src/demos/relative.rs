...
//! out of the region marked `snippet:start`/`snippet:end` below, so the
//! code on the page can never drift from what's actually compiled and
...
// Interpolating a `Vec2` field means interpolating each component; motiongfx
// only knows how to do that itself for the primitives it ships (`f64` among
// them), so a compound type spells it out once here instead of reaching for
// `act_builder().with_interp(...)` at every call site.
impl Interpolation<()> for Vec2 {
    fn interp(a: &Self, b: &Self, t: f32) -> Self {
        Vec2 { x: f64::interp(&a.x, &b.x, t), y: f64::interp(&a.y, &b.y, t) }
    }
}
...
impl SubjectSource<(), Dot> for DotWorld {
    ...
    fn apply_source<R>(&mut self, _id: (), f: impl FnOnce(&mut Dot) -> R) -> Option<R> {
        Some(f(&mut self.0))
    }
}
...
impl RelativeDemo {
    ...
    pub fn new() -> RelativeDemo {
        let dot = ();
        let world = DotWorld(Dot { position: Vec2 { x: 60.0, y: 85.0 } });
        ...
        RelativeDemo { registry, world, timeline }
    }
    ...
    #[wasm_bindgen(getter)]
    pub fn x(&self) -> f64 {
        self.world.0.position.x
    }

    #[wasm_bindgen(getter)]
    pub fn y(&self) -> f64 {
        self.world.0.position.y
    }
}
